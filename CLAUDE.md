...
- `modules/remark-wikilink-pipe-unescape/` — Nuxt module that rewrites `\|` → `|` inside `[[wiki-links]]` / `![[embeds]]` at `content:file:beforeParse` time. Works around `@flowershow/remark-wiki-link@3.4.0` consuming the `\` as a literal target char inside the link (see Pitfall #21). `scripts/validate-content.ts` imports the same `unescapeWikilinkPipes` so the validator's view of link targets matches what the renderer produces — single source of truth.
- `scripts/validate-content.ts` — the content lint (slug uniqueness, broken wikilinks, missing assets). Shares `unescapeWikilinkPipes` with the runtime; CI runs with `--strict-wikilinks` so any broken link fails the build.
...
20. **Branch merges always conflict on `content-ref.txt` unless the merge driver is registered.** master's `content-ref.txt` pins `fpwiki-content@master`; test's pins `fpwiki-content@test`; the bot bumps each independently per upstream branch — they're permanently different by design. A naive `git merge test` on master therefore conflicts on this file every single time, with the trivially-correct resolution always being "keep the current branch's value" (the bot, not git, owns content-ref.txt; it must never propagate via human merge). Auto-resolved by `.gitattributes` (`/content-ref.txt merge=ours`) plus a per-clone git-config registration of `merge.ours.driver=true` chained from `package.json postinstall` via [`scripts/setup-git.mjs`](scripts/setup-git.mjs). The driver only takes effect on clones where `pnpm install` has run at least once after the .gitattributes line landed — fresh clones are fine; clones predating the change need a one-shot `pnpm install` (or `node scripts/setup-git.mjs`) to pick up the registration. Synthetic test branches forked from a commit predating the .gitattributes change will still conflict because the rule isn't visible from their working tree at merge time — that's not a bug in the driver, it's how git resolves attributes.
21. **`@flowershow/remark-wiki-link@3.4.0` ignores `\|` escape inside `[[…]]`.** Obsidian convention requires `[[slug\|alias]]` inside GFM table cells so the `|` doesn't split the cell — `micromark-extension-gfm-table` correctly recognises `\|` as an escaped pipe and keeps the cell whole. But the wiki-link tokenizer in `node_modules/@flowershow/remark-wiki-link/dist/index.js` `inTarget`/`inAlias` consumes characters via `effects.consume(code)` without any `code === codes.backslash` branch, so the `\` becomes part of the target string. `[[isms\|ISMS]]` then yields a `wikiLink` node with `value: 'isms\\'`, `alias: 'ISMS'`, the slug doesn't resolve, and the link renders with the grey-dotted `wikilink-broken` class. The same breakage occurs in pure prose / list contexts, since the tokenizer is context-free. Fix: [`modules/remark-wikilink-pipe-unescape/`](modules/remark-wikilink-pipe-unescape/index.ts) registers a `content:file:beforeParse` hook that rewrites `\|` → `|` inside every `[[…]]` and `![[…]]` group, skipping fenced code blocks. [`scripts/validate-content.ts`](scripts/validate-content.ts) imports the same `unescapeWikilinkPipes` so the validator and the renderer agree on the resolved target — DRY, single source of truth. CI runs `pnpm content:validate --strict-wikilinks` to fail the build on any remaining broken link. Other in-repo wikilink regexes ([`modules/wiki-slug-index/build.ts WIKILINK_RE`](modules/wiki-slug-index/build.ts), `parseIndexDescriptions`, `stripWikilinks`) already normalise `\|` as an alias divider, so post-fix they agree with the renderer without further changes. Module ordering rule applies: `remark-wikilink-pipe-unescape` must sit before `@nuxt/content` in `nuxt.config.ts modules` so its `beforeParse` hook fires before the parser snapshots the body.
