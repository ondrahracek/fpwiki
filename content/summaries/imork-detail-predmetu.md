---
title: "ImorK — Detail předmětu"
course: imork
type: summary
tags: [isms, informacni-bezpecnost, oborova-reseni]
sources: [raw/imork/Detail předmětu.md]
created: 2026-04-12
updated: 2026-04-12
---

# ImorK — Detail předmětu

## Základní informace

| Položka | Hodnota |
|---------|---------|
| Název | Management oborových řešení |
| Zkratka | [[imork|ImorK]] |
| Garant | Ing. Petr Novák |
| Vyučující | Ing. Petr Sedlák |
| Prerekvizita | ImibePA |
| Zakončení | Zkouška |
| Přednášek | 13 |

## Zaměření předmětu

Předmět se zabývá implementací [[isms|ISMS]] (Information Security Management System) v různých odvětvích. Základem je rodina norem **ISO/IEC 27000**, která poskytuje rámec pro řízení informační bezpečnosti napříč sektory.

## Témata přednášek

1. Manažerská [[informacni-bezpecnost|informační bezpečnost]]
2. [[sae|SAE]] — budování bezpečnostního povědomí
3. [[rizeni-rizik|Risk management]]
4. [[isms|ISMS]] v akademickém prostředí
5. ISMS ve finančním sektoru
6. ISMS ve zdravotnictví
7. ISMS v energetice
8. ISMS u ISP (poskytovatelů internetových služeb)
9. Kybernetická bezpečnost
10. [[gdpr|GDPR]] a ochrana osobních údajů
11. Bezpečnost webových aplikací
12. Bezpečnost e-mailu
13. Bezpečnost mobilních zařízení

## Klíčové normy

- **ISO/IEC 27001** — požadavky na ISMS
- **ISO/IEC 27002** — soubor opatření
- **ISO/IEC 27005** — řízení rizik
- **ISO/IEC 27014** — governance informační bezpečnosti
